...
def test_stack(shape, axis, l, device):
    ...
    print(shape, axis, l, out.numpy().shape, out_t.numpy().shape)
    assert out.numpy().shape == out_t.numpy().shape, f"out: {out.numpy().shape}, out_t: {out_t.numpy().shape}"
...
GETITEM_PARAMETERS = [
    ((4, 4), lambda x: x[0:1, 0:1]),
    ((4, 4), lambda x: x[:, :]),
    ((32, 32, 32, 32), lambda x: x[12:32, :, 3:8, 5:7]),
    ((64, 64), lambda x: x[32:, 32:]),
]
@pytest.mark.parametrize("shape, fn", GETITEM_PARAMETERS)
@pytest.mark.parametrize("device", _DEVICES, ids=["cpu", "cuda"])
def test_getitem(shape, fn, device):
    _A = np.random.randn(*shape).astype(np.float32)
    A = ndl.Tensor(nd.array(_A), device=device, requires_grad=True)
    A_t = torch.tensor(_A, requires_grad=True, dtype=torch.float32)

    B = fn(A)
    B_t = fn(A_t)

    assert B.shape == B_t.shape
    np.testing.assert_allclose(B_t.detach().numpy(), B.detach().numpy(), atol=1e-5, rtol=1e-5)
