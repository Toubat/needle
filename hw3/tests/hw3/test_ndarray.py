...
def test_compact(params, device):
    ...
    # print('------')
    # print(A.shape, A.strides, A._offset)
    # print('------')
...
matmul_dims.extend([
  (rand_int(150, 250), rand_int(150, 250), rand_int(150, 250))
  for i in range(20)
] + [(258, 257, 256), (512, 512, 512), (1024, 1024, 1024), (2048, 2048, 2048)])
...
def test_matmul(m, n, p, device):
    ...
    print(f"\nTime taken for numpy matmul: {e1 - s1:.6f}")
    print(f"Time taken for cuda matmul: {e2 - s2:.6f}")
    print(f"Cuda faster by %: {max(((e1 - s1) - (e2 - s2)) / (e1 - s1) * 100, 0):.2f}%")

    # print(((A @ B).numpy() - _A @ _B).sum())
    # print((A @ B).numpy())
    # print(_A @ _B)
    # 

    np.testing.assert_allclose((A @ B).numpy(), _A @ _B, rtol=1e-3, atol=1e-3)
